...
#define SECONDS_IN_HOUR       (unsigned long)(SECONDS_IN_MINUTE * MINUTES_IN_HOUR)
#define SECONDS_IN_DAY        (unsigned long)(SECONDS_IN_HOUR * HOURS_IN_DAY)
...
int DS3231_init(DS3231_config* pConfig) {
  ...
  uint8_t prev_control_reg = _get_reg(DS3231_CONTROL_ADDR);
  ...
  if (pConfig->pAlarm_2 != NULL) {
    ...
    _set_reg(DS3231_ALARM1_ADDR, _dec2bcd(pConfig->pAlarm_2->min));
    _set_reg(DS3231_ALARM1_ADDR + 1, _dec2bcd(pConfig->pAlarm_2->hour));
    _set_reg(DS3231_ALARM1_ADDR + 2, _dec2bcd(pConfig->pAlarm_2->day));
  ...
  _set_reg(DS3231_CONTROL_ADDR, prev_control_reg | control_reg);
...
/*
  ...
  Serial.print(ts.year); Serial.print("/"); Serial.print(ts.mon); Serial.print("/"); Serial.print(ts.day); 
  Serial.print(" - "); Serial.print(ts.hour); Serial.print(":"); Serial.print(ts.min); Serial.print(":"); Serial.println(ts.sec);
*/
...
DS3231_unix_time_t DS3231_get_datetime(void) {
  ...
  /*
  Serial.print("CTRL REG : "); Serial.println(_get_reg(DS3231_CONTROL_ADDR));
  Serial.print("STS REG : "); Serial.println(sts_reg);
*/
  if ((sts_reg & DS3231_STATUS_A1F) != 0){
    _set_reg(DS3231_STATUS_ADDR | DS3231_STATUS_A1F, 0);
    //Serial.println("Alm1 status resetted");
  }
